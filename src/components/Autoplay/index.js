...
import {getStyle,bind,unbind,pDef,tweenMove,css,htmlFontSize} from 'js/yydjs';
import './style.scss';
...
export default class Autoplay extends React.Component{
    ...
    static defaultProps={
        ...
        */
        auto:true,//是否自动播放
        width:'100%',//容器宽度，需带上单位
        height:'300px',//容器高度，需带上单位
        t:5000,//轮播间隔
        t1:500,//轮播滚动时间
        t2:250,//划屏滚动时间
        t3:2000,//划屏后轮播延迟时间
    ...
    */
    ...
    calcWidth(length=1){
        if(this.Autoplay){
            ...
            let oW=this.Autoplay.offsetWidth||0;
            let gW=parseInt(getStyle(this.Autoplay,'width'))||0;
            let dW=document.documentElement.clientWidth;
            let cW=dW<375?dW:htmlFontSize(true)*3.75;
            let iW=Math.max(oW,gW,cW);

            iW=width&&width!='100%'?parseInt(width):iW;
            return iW*length;
        }
...
const AutoplayLi=(props)=>{
    ...
    return(
        <li style={{width:state.iW+'px'}} >
            {
                item.link&&
                <Link to={item.link?item.link:''} className="AutoplayLink"></Link>
            }
